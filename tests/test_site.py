...
class MarkaAdi(unittest.TestCase):
    ...
    def test_og_gorseli_guncel_surumda_tum_sayfalarda_ayni(self):
        beklenen = "https://exchangeatlas.org/og-cover-v3.png"
        for ad in self.SAYFALAR:
    ...
    def test_alt_sayfalar_kanonik_og_ve_twitter_meta_tasiyor(self):
        ...
        for ad, url in beklenen.items():
            t = self.sayfa(ad)
            with self.subTest(sayfa=ad):
                self.assertIn(f'<meta property="og:url" content="{url}">', t)
                self.assertIn('<meta name="twitter:title" content="Exchange Atlas · Erasmus+', t)
                self.assertIn('<meta name="twitter:description" content="', t)
...
class YayinaHazirlikIcerigi(unittest.TestCase):
    ...
    def test_anlasma_ekrani_okuma_rehberini_iki_dilde_tasiyor(self):
        ...
        self.assertIn("Starred degree badges indicate shared quotas", js)
    ...
    def test_marmara_kisa_adi_tum_yuzeylerde_mu(self):
        unis = json.loads((ROOT / "site" / "universities.json").read_text(encoding="utf-8"))
        marmara = next(u for u in unis if u["id"] == "marmara")
    ...
    def test_bagimsizlik_uyarisi_virgul_ve_ilgili_birim(self):
        dosyalar = [APP_JS, ROOT / "site" / "index.html", ROOT / "site" / "guide.html"]
        for yol in dosyalar:
            metin = yol.read_text(encoding="utf-8")
            with self.subTest(dosya=yol.name):
                self.assertIn("bağımsız bir platformdur, resmî bir üniversite hizmeti değildir", metin)
                self.assertNotIn("bağımsız bir platformdur; resmî", metin)
                self.assertIn("Erasmus+ koordinatörlüğüne, uluslararası ilişkiler birimine veya diğer ilgili birime danışın", metin)

    def test_universite_talebi_konu_satiri_buyuk_harfle(self):
        home = (ROOT / "site" / "home.js").read_text(encoding="utf-8")
        self.assertIn("Exchange Atlas: Üniversite talebi", home)
        self.assertNotIn("Exchange Atlas: üniversite talebi", home)

    def test_okuma_rehberi_seviyeleri_acikliyor(self):
        html = (ROOT / "site" / "agreements.html").read_text(encoding="utf-8")
        js = APP_JS.read_text(encoding="utf-8")
        beklenen = "ÖL ön lisans, sayıdan sonra gelen L lisans, YL yüksek lisans, D doktora anlamına gelir"
        self.assertIn(beklenen, html)
        self.assertIn(beklenen, js)
